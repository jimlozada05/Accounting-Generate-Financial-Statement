...
<div id="main">
  ...
  <section id="top" class="one">
  	...
  	<%
  		...
  		OperatingActivity oa = OperatingActivityDAO.getOneRecordByJournal(Integer.parseInt(request.getParameter("journal_id")));
  		request.setAttribute("oa",oa);
  		
  		List<CurrentAssetTitle> currentAssetTitle = Arrays.asList(CurrentAssetTitle.values());
  		request.setAttribute("currentAssetTitle", currentAssetTitle);
  		
  		List<CurrentLiabilityTitle> currentLiabilityTitle = Arrays.asList(CurrentLiabilityTitle.values());
  		request.setAttribute("currentLiabilityTitle", currentLiabilityTitle);
  	%>
  	...
  	<table class="table table-bordered">
		...
		<c:forEach items="${list}" var="list">
  			<tr>
	  				<c:if test="${list.getAccount_title() == AccountTitle.ACCUMULATED_DEPRECIATION}">
		...
		<tr><td colspan="2">+</td></tr>	
  		...
  		<c:forEach items="${list}" var="list">
  			<tr>
	  			<c:forEach var="currentAssetTitle" items="${currentAssetTitle}">
	  				<c:if test="${list.getAccount_title() == currentAssetTitle.toString()}">
		  			<td>${list.getAccount_title().str}</td>
		  			<c:if test="${list.getTotal() >= 0 }"><td>${list.getTotal() * -1}</td></c:if>
		  			<c:if test="${list.getTotal() < 0 }"><td>${list.getTotal()}</td></c:if>
		  			</c:if>
  		...
  		<c:forEach items="${list}" var="list">
  			<tr>
	  			<c:forEach var="currentLiabilityTitle" items="${currentLiabilityTitle}">
	  				<c:if test="${list.getAccount_title() == currentLiabilityTitle.toString()}">
		  			<td>${list.getAccount_title().str}</td>
		  			<c:if test="${list.getTotal() >= 0 }"><td>${list.getTotal()}</td></c:if>
		  			<c:if test="${list.getTotal() < 0 }"><td>${list.getTotal() * -1}</td></c:if>
		  			</c:if>
	...
	<tr><td colspan="2">=</td></tr>
			...
			<tr>
				...
				<td><%=oa.getOperating_activity()%></td>
			</tr>
  	</table>
