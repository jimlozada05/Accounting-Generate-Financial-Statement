	 		...
	 		<tbody>
	 			<c:forEach items="${gjList}" var="journal">
		 			<tr>
			 			...
			 			<td><a href="#" data-toggle="modal" data-target="#EditEntry${journal.getEntry_id()}">Edit</a> | <a href="webpages/DeleteJournal.jsp?entry_id=${journal.getEntry_id()}&account_title=${journal.getAccount_title()}&journal_id=${journal.getJournal_id()}">Delete</a></td>
		 			</tr>
		 			
		 				
		<div class="modal fade" id="EditEntry${journal.getEntry_id()}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content">
					<div class="modal-header">
			        ...
			        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
			          <span aria-hidden="true">&times;</span>
			      		...
			      		<div class="modal-body">
			        		<jsp:include page="webpages/editgeneraljournalform.jsp?entry_id=${journal.getEntry_id()}"></jsp:include>
			      		</div>

				</div>
			</div>
		</div>
		 			<input type="hidden" id="num" value="${journal.getEntry_id()}">
			...
			<button type="submit" onclick="CountRow()" data-toggle="modal" data-target="#AddEntry" class="btn btn-outline-dark" style="position:absolute;left:55%;padding:1% 3% 1% 3%;">Add Entry</button>

	<div class="modal fade" id="AddEntry" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content">
					<div class="modal-header">
			        ...
			        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
			          <span aria-hidden="true">&times;</span>
			      		...
			      		<div class="modal-body">
			        		<jsp:include page="webpages/generaljournaladdform.jsp?journal_id=${param.journal_id}"></jsp:include>
			      		</div>
	 	...
	 	<form method="post" action="webpages/FinishJournal.jsp">
			...
			<button type="submit" class="btn btn-outline-success" style="position:absolute;right:10%;padding:1% 1.5% 1% 1.5%;">Finalize &amp; Submit</button>
		</form>
	 	
	 	<form method="post" action="webpages/deletegroup.jsp">
			<input type="hidden" name="journal_id" value="<%out.print(request.getParameter("journal_id"));%>">
			<button type="submit" class="btn btn-outline-danger" style="position:absolute;right:.5%;padding:1% 3% 1% 3%;">Cancel</button>
		</form>
  </section>
