...
<div id="main">
  ...
  <section id="top" class="one">
  	...
  	<%
  		...
  		List<RevenueTitle> revenueTitle = Arrays.asList(RevenueTitle.values());
  		request.setAttribute("revenueTitle", revenueTitle);
  		
  		List<ExpenseTitle> expenseTitle = Arrays.asList(ExpenseTitle.values());
  		request.setAttribute("expenseTitle", expenseTitle);
  	%>
  	...
  	<table class="table table-bordered">
  		...
  		<c:forEach items="${list}" var="list">
  			<tr>
	  			<c:forEach var="revenueTitle" items="${revenueTitle}">
	  				<c:if test="${list.getAccount_title() == revenueTitle.toString()}">
  		...
  		<c:forEach items="${list}" var="list">
  			<tr>
	  			<c:forEach var="expenseTitle" items="${expenseTitle}">
	  				<c:if test="${list.getAccount_title() == expenseTitle.toString()}">
		  			<td>${list.getAccount_title().str}</td>
		  			<c:if test="${list.getTotal() >= 0 }"><td>${list.getTotal()}</td></c:if>
		  			<c:if test="${list.getTotal() < 0 }"><td>${list.getTotal() * -1}</td></c:if>
		  			</c:if>
