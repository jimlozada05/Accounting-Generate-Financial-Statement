...
<div id="main">
  ...
  <section id="top" class="one">
  	...
  	<%
  		Map<TrialBalance, List<GeneralLedger>> tbs = TrialBalanceDAO.getTrialBalanceWithLedger(Integer.parseInt(request.getParameter("journal_id")));
  		request.setAttribute("tbs", tbs);
  		
  		TrialBalance ttb=TrialBalanceDAO.getABalance(Integer.parseInt(request.getParameter("journal_id")));
  	...
  	<table class="table table-bordered">
  		...
  		<tbody>
  	<c:forEach items="${tbs.keySet()}" var="tb">
  		<c:forEach items="${tbs.get(tb)}" var="ledger">
  		<tr>
  			<td>${ledger.account_title.str}</td>
  		...
  		<tr>
  			<td>Total</td>
  			<td><%=ttb.getTotal_debit()%></td>
  			<td><%=ttb.getTotal_credit() * -1 %></td>
  		</tr>
  		</tbody>
  	</table>
