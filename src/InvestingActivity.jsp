...
<div id="main">
  ...
  <section id="top" class="one">
  	...
  	<%
  		List<JournalEntry> list =JournalEntryDAO.getAllRecordWithCash(Integer.parseInt(request.getParameter("journal_id")));	
  	...
  	<table class="table table-bordered">
		...
		<c:forEach items="${list}" var="list">
  			...
  			<c:forEach var="assetTitle" items="${assetTitle}">
	  				<c:if test="${list.getAccount_title() == assetTitle.toString()}">
		  			<td>${list.getAccount_title().str}</td>
		  			<c:if test="${list.getDebit() >= 0 }"><td>${list.getDebit() * -1}</td></c:if>
		  			<c:if test="${list.getCredit() < 0 }"><td>${list.getCredit()}</td></c:if>
		  			</c:if>
		  	</c:forEach>
  			</tr>
  		</c:forEach>
